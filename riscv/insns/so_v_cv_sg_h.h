...
auto baseBehaviour = [](auto &dest, auto &src, auto extra) {
    ...
    for (size_t i = 0; i < finalElementCount; ++i)
        out.at(i) = readAS<StorageType>(static_cast<short int>(readAS<OperationType>(elements.at(i))));
...
std::visit(overloaded{
    [&](StreamReg16 &dest, StreamReg8 &src) { baseBehaviour(dest, src, (signed char){}); },
    [&](StreamReg16 &dest, StreamReg16 &src) { baseBehaviour(dest, src, (short int){}); },
    [&](StreamReg16 &dest, StreamReg32 &src) { baseBehaviour(dest, src, (int){}); },
    [&](StreamReg16 &dest, StreamReg64 &src) { baseBehaviour(dest, src, (long int){}); },
    [&](auto &dest, auto &src) { assert_msg("Invoking so.v.cv.sg.h with invalid parameter sizes", false); }
}, destReg, srcReg);