...
auto baseBehaviour = [](auto &dest, auto &src, auto extra) {
    ...
    for (size_t i = 0; i < finalElementCount; ++i)
        out.at(i) = readAS<StorageType>(static_cast<unsigned long int>(readAS<OperationType>(elements.at(i))));
...
std::visit(overloaded{
    [&](StreamReg64 &dest, StreamReg8 &src) { baseBehaviour(dest, src, (unsigned char){}); },
    [&](StreamReg64 &dest, StreamReg16 &src) { baseBehaviour(dest, src, (unsigned short int){}); },
    [&](StreamReg64 &dest, StreamReg32 &src) { baseBehaviour(dest, src, (unsigned int){}); },
    [&](StreamReg64 &dest, StreamReg64 &src) { baseBehaviour(dest, src, (unsigned long int){}); },
    [&](auto &dest, auto &src) { assert_msg("Invoking so.v.cv.us.d with invalid parameter sizes", false); }
}, destReg, srcReg);