...
auto baseBehaviour = [](auto &dest, auto &src, auto extra) {
    ...
    for (size_t i = 0; i < finalElementCount; ++i)
        out.at(i) = readAS<StorageType>(static_cast<int>(readAS<OperationType>(elements.at(i))));
...
std::visit(overloaded{
    [&](StreamReg32 &dest, StreamReg8 &src) { baseBehaviour(dest, src, (signed char){}); },
    [&](StreamReg32 &dest, StreamReg16 &src) { baseBehaviour(dest, src, (short int){}); },
    [&](StreamReg32 &dest, StreamReg32 &src) { baseBehaviour(dest, src, (int){}); },
    [&](StreamReg32 &dest, StreamReg64 &src) { baseBehaviour(dest, src, (long int){}); },
    [&](auto &dest, auto &src) { assert_msg("Invoking so.v.cv.sg.w with invalid parameter sizes", false); }
}, destReg, srcReg);